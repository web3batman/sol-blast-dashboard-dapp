  ...
  return (
    <>
      ...
      <div className="flex lg:hidden">
        ...
        <div
          className={clsx(
            'absolute -right-full top-0 z-[998] max-h-screen h-full w-screen overflow-hidden bg-[rgba(0,0,0,0.2)] backdrop-blur-[5px] transition-all',
            activeToggle ? '-translate-x-full' : 'translate-x-0 hidden',
          )}>
          <div className="absolute right-0 top-0 h-screen w-80 bg-black">
            <div className="flex flex-col items-center gap-2 pt-20 text-sm text-black">
              <MobileButton
                title="Home"
                isActive={activePath === '/'}
                onClick={() => { router.push('/'); setActiveToggle(false); }}
              />

              <MobileButton
                title="Leaderboard"
                isActive={activePath === '/leaderboard'}
                onClick={() => { router.push('/leaderboard'); setActiveToggle(false); }}
              />

              <MobileButton
                title="About"
                isActive={activePath === '/about'}
                onClick={() => { router.push('/about'); setActiveToggle(false); }}
              />

              <MobileButton
                title="Rewards"
                isActive={activePath === '/rewards'}
                onClick={() => { router.push('/rewards'); setActiveToggle(false); }}
              />
            </div>
          </div>
        </div>
